...
PictureList::PictureList(QWidget *parent,bool flag,int borNum)
{
    head=new NodeP;
    this->length=0;
    this->flag=flag;
    ...
    QStringList p[21];
    readFile(p);
    int code,pageNum,isbn;
    Pictures book;
    QString author,title,pubHouse,lever,status;
    for(int i=0;i<20;i++)
    {
        qDebug()<<p[i];
        code=p[i].at(0).toInt();
        ...
        book.create(pubHouse,isbn,pageNum,code,title,author,status,lever);
        this->insert(book);
    }
    ...
    headText<<"编码"<<"标题"<<"作者"<<"国家"<<"长度"<<"宽度"<<"状态"<<"评级"<<"操作";
...
void PictureList::outFile()
{
    QFile aFile("C:\\Users\\哈默默\\Desktop\\pictData.csv");
    QTextStream out(&aFile);
    if (!aFile.open(QIODevice::WriteOnly | QIODevice::Text));
    NodeP* p=this->head->next;
    while(p!=NULL)
    {
      ...
      str4=p->book.getItems().country;
      str5=QString::number(p->book.getItems().height);
      str6=QString::number(p->book.getItems().weight);
...
void PictureList::insert(Pictures book)
{
    NodeP* p=this->head;
    NodeP *q=new NodeP(book);
    while(p->next!=NULL)
    {
      p=p->next;
    }
    ...
    for(int j=0;j<this->length;j++)
    {
        ...
        for(int i=0;i<9;i++)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                item=new QTableWidgetItem(bokItem.country);
                item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
                table->setItem(j,i,item);
                break;
            case 4:
                item=new QTableWidgetItem(QString::number(bokItem.height));
                item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
                table->setItem(j,i,item);
                break;
            case 5:
                item=new QTableWidgetItem(QString::number(bokItem.weight));
                item->setTextAlignment(Qt::AlignHCenter | Qt::AlignVCenter);
                table->setItem(j,i,item);
                break;
            case 6:
...
void PictureList::borBook(int code)
{
    NodeP *p=head->next;
    while(p!=NULL){
...
void PictureList::retBook(int code)
{
    NodeP *p=head->next;
    while(p!=NULL){
        if(p->book.getItems().code==code)
...
void PictureList::readFile(QStringList p[])
{
    QFile  aFile("C:\\Users\\哈默默\\Desktop\\PictureData.txt");
    int i=0;
...
void PictureList::searchByCode(int code,QWidget *parent)
{
    table->clearContents();
...
void PictureList::searchByTitle(QString title,QWidget *parent)
{
    table->clearContents();
...
void PictureList::setLend(int code)
{
    NodeP *p=head->next;
    while(p!=NULL){
        if(p->book.getItems().code==code)
...
void PictureList::delItem(int code)
{
    NodeP *p=head->next;
    NodeP *q;
